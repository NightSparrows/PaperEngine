...
namespace PaperEngine {
	...
	SceneRenderer::SceneRenderer()
	{
		...
		for (uint32_t i = 0; i < Application::Get()->getGraphicsContext()->getMaxFrameInFlight(); i++)
		{
			...
			globalSetDesc.addItem(nvrhi::BindingSetItem::StructuredBuffer_SRV(0, m_lightCullPass.getDirectionalLightBuffer()->getStorages()[i].buffer));
			globalSetDesc.addItem(nvrhi::BindingSetItem::StructuredBuffer_SRV(1, m_lightCullPass.getPointLightBuffer()->getStorages()[i].buffer));
			globalSetDesc.addItem(nvrhi::BindingSetItem::StructuredBuffer_SRV(2, m_lightCullPass.getPointLightCullData().globalLightIndicesBuffer->getStorages()[i].buffer));
			globalSetDesc.addItem(nvrhi::BindingSetItem::StructuredBuffer_SRV(3, m_lightCullPass.getPointLightCullData().clusterRangesBuffer->getStorages()[i].buffer));
