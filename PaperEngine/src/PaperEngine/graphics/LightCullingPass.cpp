...
#pragma endregion
		...
		m_pointLightCullData.resize(Application::Get()->getGraphicsContext()->getSwapchainCount());
		for (auto& pointLightCullData : m_pointLightCullData)
		{
...
#pragma region Global data GPU Buffer
			{
				...
				bufferDesc
					...
					.setIsConstantBuffer(true)
					.setKeepInitialState(true);
				pointLightCullData.globalDataBuffer = Application::GetNVRHIDevice()->createBuffer(bufferDesc);
				pointLightCullData.globalDataBufferPtr = Application::GetNVRHIDevice()->mapBuffer(pointLightCullData.globalDataBuffer, nvrhi::CpuAccessMode::Write);
			}
#pragma endregion
...
#pragma region Global Light Indices Buffer Creation
			{
				...
				pointLightCullData.globalLightIndicesBuffer = Application::GetNVRHIDevice()->createBuffer(bufferDesc);
			}
#pragma endregion
...
#pragma region Cluster Ranges Buffer Creation
			{
				...
				pointLightCullData.clusterRangesBuffer = Application::GetNVRHIDevice()->createBuffer(bufferDesc);
			}
#pragma endregion
...
#pragma region Binding Set Creation
			{
				...
				bindingSetDesc
					.addItem(nvrhi::BindingSetItem::ConstantBuffer(0, pointLightCullData.globalDataBuffer))
					.addItem(nvrhi::BindingSetItem::StructuredBuffer_SRV(0, m_pointLightBuffer))
					.addItem(nvrhi::BindingSetItem::StructuredBuffer_UAV(0, pointLightCullData.globalLightIndicesBuffer))
					.addItem(nvrhi::BindingSetItem::StructuredBuffer_UAV(1, pointLightCullData.clusterRangesBuffer))
					.addItem(nvrhi::BindingSetItem::RawBuffer_UAV(2, pointLightCullData.globalCounterBuffer));
				pointLightCullData.lightCullBindingSet = Application::GetNVRHIDevice()->createBindingSet(
					bindingSetDesc,
...
#pragma endregion
	...
	{
		GlobalData* globalData = static_cast<GlobalData*>(m_pointLightCullData[Application::Get()->getGraphicsContext()->getSwapchainIndex()].globalDataBufferPtr);
